const jsPsych = initJsPsych({
  default_iti: 1000,
  on_finish: function() {
    jsPsych.data.get().localSave('csv','Evaluation.csv');
  }
});
...
let demo_info = {
    ...
    questions: [
      {
        prompt:
          "<p>欢迎参加本次实验！</p>" +
          "<p>这是一项有关金钱偏好的研究。" +
          "在每个试次中，你将看到两个涉及金钱损失的选项。你需要对其中的一个选项进行设定，使得你愿意接受这两个选项的程度相同。比如，假设你愿意接受“4星期后损失100元”和 “现在损失50元”的程度相同，那么当被问到你对于“现在损失多少元”与“4星期后损失100元”会有相同的接受程度时，你应该回答现在损失50元。一些情况下你可能愿意在较近的时刻接受更大的损失，这也是被允许的，根据你自身的想法作答即可。当然，当被问到你对于“现在损失200元”和“现在损失多少元”会有相同的接受程度这样的问题时，由于两个选项的延迟时长相同，此时你应该回答与另一选项相同的金额，即应该回答现在损失200元。总之，你的任务是使得你愿意接受两个选项的程度相同。</p> " 
  ...
  const set_clozeText = function (trialdata) {
    ...
    const option_to_match = `现在损失 % ${lm} % 元`;
    let option_given;
    if (ld == 0) {
        option_given = `现在损失 ${lm} 元`;
    } else {
        option_given = `${ld} 星期后损失 ${lm} 元`;
    }
